...
system_create_user() {
  print_banner
  printf "${WHITE} 💻 Agora, vamos criar o usuário para deploynextchat...${GRAY_LIGHT}"
  printf "\n\n"

  sleep 2
 
  sudo su - root <<EOF
  useradd -m -p $(openssl passwd $deploy_password) -s /bin/bash -G sudo deploynextchat
  usermod -aG sudo deploynextchat
  mv "${PROJECT_ROOT}"/whaticket.zip /home/deploynextchat/
EOF
...
system_node_install() {
  ...
  sudo -u postgres psql -c "CREATE DATABASE whaticketnextchat;"
  exit
EOF
...
system_pm2_install() {
  ...
  sudo su - root <<EOF
  npm install -g pm2
  pm2 startup ubuntu -u deploynextchat
  env PATH=\$PATH:/usr/bin pm2 startup ubuntu -u deploynextchat --hp /home/deploynextchat
EOF
