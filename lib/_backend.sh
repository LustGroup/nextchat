...
backend_redis_create() {
  ...
  sudo su - root <<EOF
  usermod -aG docker deploynextchat
...
sudo su - deploynextchat << EOF
  cat <<[-]EOF > /home/deploynextchat/whaticket/backend/.env
NODE_ENV=
...
backend_start_pm2() {
  ...
  sudo su - deploynextchat <<EOF
  cd /home/deploynextchat/whaticket/backend
  pm2 start nextchat/server.js --name whaticket-backend
EOF
